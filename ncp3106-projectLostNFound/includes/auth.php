...
function registerUser($userData)
{
    ...
    // Validate input
    if (
        empty($userData['username']) || empty($userData['full_name']) ||
...
function loginUser($email, $password)
{
    ...
    // Get user by email
    $stmt = $conn->prepare("SELECT id, username, full_name, email, phone, department, password, role FROM users WHERE email = ? LIMIT 1");
    ...
    if (password_verify($password, $user['password'])) {
        ...
        error_log('Login successful for user (email): ' . $user['email']);
